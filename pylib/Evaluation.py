...
def get_preds(scores):
    ...
    '''
    #score shape is batch_size x 68 x 64 x 64
    assert scores.dim() == 4, 'Score maps should be 4-dim'

    # scores is flattenned to get the maximum value in each heatmap
    maxval, idx = torch.max(scores.view(scores.size(0), scores.size(1), -1), 2) 
    
    #scores.view(scores.size(0), scores.size(1), -1).shape = batch_size x 68 x 4096(64*64)
    #maxval shape = batch_size x 68
    #idx shape = batch_size x 68    

    #print('0 is here ', scores.shape)
    #print('1 is here ', scores.view(scores.size(0), scores.size(1), -1).shape)
    #print('maxval ', maxval,idx)

    maxval = maxval.view(scores.size(0), scores.size(1), 1) # batch_size x 68 x 1
    idx = idx.view(scores.size(0), scores.size(1), 1) + 1   # batch_size x 68 x 1? why plus 1 (later we are subtracting 1)

    #print('maxval shape', maxval.shape,idx.shape)

    # preds keep the row and column index of the maximum in the heatmap
    preds = idx.repeat(1, 1, 2).float() # batch_size x 68 x 2
    
    #scores size = batch_size x 68 x 64 x 64
    #scores.size(3) = 64
    #scores.size(2) = 64
    preds[:,:,0] = (preds[:,:,0] - 1) % scores.size(3) + 1
    preds[:,:,1] = torch.floor((preds[:,:,1] - 1) / scores.size(2)) + 1

    # First maskval is made exactly as the same shape as pred_mask.
    # We consider only those values of row and column indices in pred where
    # maxval is greater than zero
...
def transform_preds(coords, center, scale, res, rot):
    ...
    # exit()
    #print('-------------', type(center),type(scale),type(rot))
    center = center.numpy()
    #scale = scale.numpy()
    #rot = rot.numpy()
...
def GetTransform(center, scale, rot, res, size):
    #print('size and scale', size, scale)
    # Generate transformation matrix
    #h = size * scale #wenxuan modified to the 3 lines below
    h = size * scale*1.0
    if h == 0:
        h = h+0.000000000000000000001  

     # size_src = size_dst * scale
    t = np.zeros((3, 3))
    # print res, float(res), type(res), float(res) / h
    t[0, 0] = float(res) / h
    t[1, 1] = float(res) / h
    t[0, 2] = res * (-float(center[0]) / h + .5)
    t[1, 2] = res * (-float(center[0]) / h + .5) #wenxuan center[1] can not be found, is changed to cente[1]
    t[2, 2] = 1
    if not rot == 0:
