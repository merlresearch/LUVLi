...
def GetTransform(center, scale, rot, res, size):
    ...
    # If rotation is there first translate at center, then rotate and then translate
    ...
    if not rot == 0:
        ...
        # Inverse translation, rotation, translation and original scaling
...
def TransformSinglePts(pts, center, scale, rot, res, size, invert=0):
    ...
    # DO NOT round off to integer. Rounding the landmarks to integers introduces quantization error.
...
def TransformImg(img, center, scale, rot, res, size):
    ...
    # Upper left point
    ul = np.array(TransformSinglePts([0,0], center, scale, 0, res, size, invert=1)).astype(int)
    # Bottom right point
    br = np.array(TransformSinglePts([res,res], center, scale, 0, res, size, invert=1)).astype(int)
...
def crop(img, center, scale, rot, res, size):
    """
        Crops the images with given parameters
        Called with scale = random number, rot = random number
        res = 256
        size = 200
    """
    # This scale factor actually scales down the images. It is used for division
    scale_factor = float(scale * size) / float(res)

    # ==========================================================================
    # Abhinav: Unclear why this clamping is being done.
    # Kept this since all experiments were done using this.
    # ==========================================================================
    #"""
    if scale_factor < 2:
        #print("Going here")
    ...
    try:
        ...
        # Sometimes image is not read and the above line throws an exception
        new_img[new_y[0]:new_y[1], new_x[0]:new_x[1]] = 0
...
def shufflelr(x, width, dataset= 'MPII'):
    ...
    if ((dataset ==  'MPII') or (dataset == 'LEEDS')):
        if (numClasses == 16):
            matchedParts = (
                [0,5],   [1,4],   [2,3],
                [10,15], [11,14], [12,13]
            ...
            print("Unknown number of landmark points.")
    elif dataset == 'face':
        matchedParts = (
...
def resize_4d_numpy(input, downsample= 4):
    """
        Converts batch of heatmaps to a different sampled version
    """
